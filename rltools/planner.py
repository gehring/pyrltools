...
class SingleEmbeddedAgent(PlanningAgent):
    def __init__(self,
        ...
        if X_t is None:
            self.model = SingleCompressModel(dim = dim,
                                ...
                                differential_model = self.use_diff_model)
    ...
    def reset(self):
        self.plan = np.ones((self.H, self.plan.shape[1]))/self.plan.shape[1]
        
        if self.use_valuefn:
            self.plan[0,:] = 1.0/(self.plan.shape[1]-1)
            self.plan[0,-1] = 0
    ...
    def get_action(self, t = 0):
        ...
        return self.actions[np.digitize(np.random.random_sample(1), bins)]
        ...
        for t in xrange(H-2, -1, -1):
            for a in xrange(k):
                ...
                if self.use_diff_model:
                    alphas[a][t,:] += sum( [ self.discount * new_plan[t+1, b] * alphas[b][t+1,:].dot(Vab[b,a]) for b in xrange(k)])
                vals[a] = alphas[a][t,:].dot(betas[a][t,:])
...
class CompressedLEMAgent(PlanningAgent):
    def __init__(self,
                 plan_horizon,
                 dim, 
                 X_t,
        ...
        self.action_kernels = action_kernels
        self.alphas = np.zeros((plan_horizon+1,dim))
        self.betas = np.zeros((plan_horizon,dim))
        self.explicit_models = (None, None)
        Fa, wa = self.explicit_models
        self.explicit_models = self.model.generate_explicit_model(
            action_kernels, 
            Fa, 
            wa, 
            value_fn = self.theta if self.use_valuefn else None)
    ...
    def step(self, x_t, a_t, r_t, x_tp1):
        ...
        if x_t is not None and self.use_valuefn:
            ...
            deltas = np.sum(self.betas * (self.alphas[1:,:] - self.theta[None,:]), axis=1) * (learning_rate_decay ** np.arange(self.H))
            dtheta = np.sum(self.betas * deltas[:,None], axis = 0)
            self.theta += self.learning_rate/2 * dtheta
...
#            self.theta += self.learning_rate/2 * delta * phi_t
    ...
    def improve_plan(self):
            ...
            H = self.H
            k = self.num_actions
            alphas[H,:] =  wa[-1] if self.use_valuefn else 0 # assuming terminal reward is zero if not using value function
            betas[0,:] = self.phi(self.x_t)
            ...
            while np.abs(diff) > np.abs(0.01*value):
                old_plan = self.plan.copy()
                self.plan, old_val = self.forward_step(alphas, betas, self.plan, improve = True)
                self.plan, new_val = self.backward_step(alphas, betas, self.plan, improve = True)
                diff = new_val - old_val
    ...
    def forward_step(self, alphas, betas, plan, improve):
            ...
            for t in xrange(1, H):
                betas[t,:] = sum( [new_plan[t-1, b] * Fa[b].dot(betas[t-1,:]) for b in xrange(k)])
                for a in xrange(k):
                    vals[a] =(discount*alphas[t+1,:].dot(Fa[a]) + wa[a]).dot(betas[t,:])
